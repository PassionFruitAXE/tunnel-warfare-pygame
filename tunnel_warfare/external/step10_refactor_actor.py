...
from pythonProject.actor.prizes import PrizesSprite
...
def step10_refactor_actor():
    ...
    # 加载图片文件
    background = pygame.image.load("../resources/tmx/map_1.png")
    ...
    while True:
        ...
        for japanese in japanese_group:
            japanese.draw(screen,view_x,view_y)
        tie_tou.draw(screen,view_x,view_y)
        jian_jun.draw(screen,view_x,view_y)
        for prizes in prizes_group:
            prizes.draw(screen,view_x,view_y)

        xiao_tie.run(down_flag,key_list)

        for japanese in japanese_group:
            japanese.run(down_flag,key_list)
